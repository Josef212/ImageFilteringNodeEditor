...
class NAMENode(BaseNode):
    ...
    def build_dpg(self, editor):
        ...
        with dpg.node(label="NAME", parent=editor) as node:
            with dpg.node_attribute(label="InputAtr", attribute_type=dpg.mvNode_Attr_Input) as in_atr:
                dpg.add_input_int(label="FIELD1", width=150, callback=update_value, user_data=(self, 1), default_value=self.field1)
                dpg.add_input_int(label="FIELD2", width=150, callback=update_value, user_data=(self, 2), default_value=self.field2)
